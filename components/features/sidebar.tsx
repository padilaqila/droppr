...
export function Sidebar({ isMobileOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Main Sidebar Aside (Desktop Fixed, Mobile Slide-over Drawer) */}
      <aside
        className={`fixed left-0 top-0 bottom-0 w-64 bg-bg-sidebar border-r border-border-hairline flex flex-col z-50 md:z-30 select-none transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="h-14 px-4 flex items-center justify-between border-b border-border-subtle">
          ...
          <button
            type="button"
            onClick={onClose}
            className="md:hidden p-1.5 rounded-md text-text-tertiary hover:text-text-primary hover:bg-bg-elevated transition-colors"
